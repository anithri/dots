...
TXT
  ...
  git :init
  git :add => '.'
  git :commit => "-aqm 'Generate a new Rails application.'"

  # Add the 'develop' branch for working copy
...
@current_recipe = "minitest"
@before_configs["minitest"].call if @before_configs["minitest"]
say_recipe 'minitest'

config = {}
config['minitest'] = multiple_choice(
  "Choose your testing framework.",
  [
    ["Standard Test::Unit", false],
    ["MiniTest", "minitest"],
    ["MiniTest with VCR", "minitest-vcr"],
    ["RSpec", 'rspec'],
    ["RSpec with VCR", 'rspec-vcr']
  ]
)
@configs[@current_recipe] = config

case config['minitest']
  when "minitest"
    recipes << ['minitest']
  when "minitest-vcr"
    recipes << ['minitest', 'vcr']
  when "rspec"
    recipes << ['rspec']
  when "rspec-vcr"
    recipes << ['rspec', 'vcr']
  else
    say_wizard "Using standard Rails Test::Unit"
end
...
if recipes.include? 'vcr'
  gem_group :test do
    gem 'vcr'
    gem 'webmock'
  end

  test_dir = if recipes.include? 'rspec'
               "spec"
             else
               "test"
             end

  create_directory "#{test_dir}/fixtures/cassettes"

  prepend_file "#{test_dir}/#{test_dir}_helper" do <<-RUBY
  VCR.configure do |c|
    c.cassette_library_dir = '#{test_dir}/fixtures/cassettes'
...
@before_configs["generators"].call if @before_configs["generators"]
say_recipe 'generators'

test_framework = if recipes.include? 'rspec'
                   "rspec"
                 else
                   "test_unit"
                 end
...
config.generators do |g|
  ...
  g.test_framework :#{test_framework}, fixtures: false, fixture_replacement: :factory_girl
end
RUBY
end
